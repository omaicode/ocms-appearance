...
namespace Modules\Appearance\Console\Generators;

use Exception;
use Modules\Appearance\Console\Commands\Traits\BlockMessage;
use Modules\Appearance\Console\Commands\Traits\SectionMessage;
...
class MakeTheme extends Command
{
     ...
     * @var string
     */
    protected $name = 'ocms:make-theme';
    ...
    {
        $this->themePath = $this->config->get('appearance.directory', 'themes');
     ...
     * Notice: if value is set in appearance.composer.author.name and appearance.composer.author.email config value
     * then this value will be used.
     */
    protected function askAuthor()
    {
        $this->theme['author-name'] = $this->config->get('appearance.composer.author.name') ?? $this->ask('Author name');
        $this->theme['author-email'] = $this->config->get('appearance.composer.author.email') ?? $this->ask('Author email');
    }
     ...
     * Notice: if value is set in appearance.composer.vendor config value
     * then this value will be used.
     */
    protected function askVendor()
    {
        do {
            $this->theme['vendor'] = mb_strtolower($this->config->get('appearance.composer.vendor') ?? $this->ask('Vendor name'));
